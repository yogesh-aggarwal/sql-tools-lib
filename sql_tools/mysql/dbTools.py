...
def createDb(db, raiseError=True, verbose=False):
    ...
    for x in db:
        try:
            tools.setStatus(
                f"Creating: {x}", verbose=verbose, raiseError=raiseError
            )
            execute(
                [f"CREATE DATABASE {x};"],
                ["mysql"],
...
def delDb(db="", raiseError=True, verbose=False):
    ...
    for x in db:
        try:
            tools.setStatus(
                f"Deleting: {x}", verbose=verbose, raiseError=raiseError
            )
            execute(
                [f"DROP DATABASE {x};"],
                ["mysql"],
...
def getDbs(raiseError=True, verbose=False):
    try:
        return [
            x[0]
            for x in execute(
                ["SHOW DATABASES;"], ["mysql"], verbose=verbose
            ).list[0][0]
        ]
    except Exception as e:
        raise e if raiseError else False
        ...
        if len(tables) != 0:
            ...
            for i, x in enumerate(tables):
                for table in x:
                    records.append(
                        execute(
                            [f"SELECT * FROM {table}"], db[i], verbose=verbose
                        ).list
                    )
