# -*- coding: utf-8 -*-
"""
Parse extention for SQL-Tools lib (pretty) (parser)
"""
...
class InvalidSQL(Exception):
    ...
    """
    def __init__(self, message="Not a valid SQL query."):
        super(InvalidSQL).__init__()
...
class InvalidSelect(InvalidSQL):
    ...
    Database exception for SQL-Tools.
    """
    def __init__(self, message="Invalid select."):
        super(InvalidSQL).__init__()
...
class InvalidCondition(InvalidSQL):
    ...
    Database exception for SQL-Tools.
    """
    def __init__(self, message="Invalid condition."):
        super(InvalidSQL).__init__()
...
class InvalidGroupBy(InvalidSQL):
    ...
    Database exception for SQL-Tools.
    """
    def __init__(self, message="Invalid group by."):
        super(InvalidSQL).__init__()
...
class InvalidFunc(InvalidSQL):
    ...
    Database exception for SQL-Tools.
    """
    def __init__(self, message="Invalid function."):
        super(InvalidSQL).__init__()
...
class InvalidLimit(InvalidSQL):
    ...
    Database exception for SQL-Tools.
    """
    def __init__(self, message="Invalid limit."):
        super(InvalidSQL).__init__()
...
class InvalidOrderBy(InvalidSQL):
    ...
    Database exception for SQL-Tools.
    """
    def __init__(self, message="Invalid order by."):
        super(InvalidSQL).__init__()
...
class UnbalancedParenthesis(InvalidSQL):
    ...
    Database exception for SQL-Tools.
    """
    def __init__(self, message="Unbalanced paranthesis."):
        super(InvalidSQL).__init__()
...
def _parse_from(toks):
    ...
    while len(toks) > i:
        if toks[i]._type in (Token.IDENTIFIER, Token.NUMBER):
            ...
            while len(toks) > i:
                if _match(
                    toks[i:], [Token.IDENTIFIER, Token.COMPARE, Token.IDENTIFIER]
                ):
                    condition = Condition(
                        [
                            Identifier(toks[i + 0]._value),
                ...
                if _match(toks[i:], [Token.FUNC]):
                    ...
                    if _match(toks[i + j :], [Token.COMPARE, Token.IDENTIFIER]):
                        ...
                        condition = Condition(
                            [func, Operator(toks[i + 0]._value), func2]
                        )
                        vals.append(condition)
                        i += 2 + j
...
def _parse_limit(toks):
    if len(toks) > 3:
        ...
        if _match(toks, [Token.LIMIT, Token.NUMBER, Token.COMMA, Token.NUMBER]):
            return (
                Limit(row_count=Number(toks[3]._value), offset=Number(toks[1]._value)),
                4,
            )
        ...
        if _match(toks, [Token.LIMIT, Token.NUMBER, Token.IDENTIFIER, Token.NUMBER]):
            if toks[2]._value.upper() == "OFFSET":
                ...
                return (
                    Limit(
                        row_count=Number(toks[1]._value),
                        offset=Number(toks[3]._value),
                        offset_keyword=toks[2]._value,
                    ),
                    4,
                )
...
def _parse_insert(toks):
    ...
    if _match(toks[i:], [Token.VALUES, Token.PARENTHESIS_OPEN]):
        ...
        while i < len(toks):
            ...
            if toks[i]._type == Token.COMMA:
                ...
                toks[i:], [Token.PARENTHESIS_CLOSE, Token.COMMA, Token.PARENTHESIS_OPEN]
            ):
    ...
    return (
        Insert(toks[0]._value, toks[1]._value, values, cols=columns, select=select),
        i,
    )
...
def _parse_case(toks):
    ...
    while i < len(toks):
        if _match(
            ...
            toks[i:], [Token.ELSE, (Token.IDENTIFIER, Token.NUMBER, Token.STR)]
        ):

            else_ = Else(toks[i + 0]._value, toks[i + 1]._value)
...
def _parse_conditions(toks):
    ...
    while i < len(toks):
        ...
        if _match(
            ...
            toks[i:],
            [(Token.IDENTIFIER, Token.NUMBER, Token.STR), Token.IS, Token.NULL],
        ):
            condition = Condition(
                [
                    _get_simple_object(toks[i]),
            ...
            condition = Condition(
                [_get_simple_object(toks[i]), Operator(toks[i + 1]._value)]
            )
            i += 3
            objects = []
